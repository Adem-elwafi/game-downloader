...
export default function Navbar() {
  ...
  return (
    <nav className="bg-[#640D5F] text-white shadow-md sticky top-0 z-50">
      <div className="max-w-7xl mx-auto px-4 flex justify-between items-center h-16">
        {/* Logo */}
        <Link to="/" className="text-2xl font-extrabold tracking-wide">
          <span className="text-[#FFCC00]">Games</span>
        ...
        <div className="hidden md:flex space-x-6 items-center">
          {navLinks.map(link => (
            <Link
              key={link.name}
              to={link.path}
              className="relative px-2 py-1 font-medium hover:text-[#EB5B00] transition duration-200"
            >
              {link.name}
              {/* underline hover effect */}
              <span className="absolute left-0 -bottom-1 w-0 h-0.5 bg-[#EB5B00] transition-all duration-300 hover:w-full"></span>
            </Link>
          ))}
          <Link
            to="/download"
